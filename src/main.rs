...
use kube::{
    ...
    {api::ListParams, client::Client, Api},
};
use kube_runtime::{
...
enum Action {
    /// Create the sub-resources, this includes spawning `n` pods with Echo service
    Create,
    /// Delete all sub-resources created in the `Create` phase
    Delete,
...
async fn reconcile(echo: Echo, context: Context<ContextData>) -> Result<ReconcilerAction, Error> {
    ...
    return match determine_action(&echo) {
        Action::Create => {
            ...
            // at any time, leaving sub-resources in intermediate state. This prevents leaks on
            // the `Echo` resource deletion.
            let name = echo.name(); // Name of the Echo resource is used to name the sub-resources as well.
        ...
        Action::Delete => {
            // Deletes any sub-resources related to this `Echo` resources. If and only if all sub-resources
...
fn determine_action(echo: &Echo) -> Action {
    let meta = echo.meta();
    return if meta.deletion_timestamp.is_some() {
        Action::Delete
